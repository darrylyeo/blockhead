...
<SizeContainer class="stack align-{align}" {...$$restProps}>{#key key ?? value}{#if transition}<span class="column align-{align}" transition:transition={transitionParams}><slot {key} {value}>{value}</slot></span>{/if}{/key}</SizeContainer>
