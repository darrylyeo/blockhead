<script lang="ts">
	...
	let {
		...
		errorMessage: string
		errorFunction?: ((error: LoaderError) => string) | undefined
		hideError?: boolean

		fromPromise?: (() => Promise<LoaderResult>) | undefined
		fromStore?: (() => Readable<ApolloResult<LoaderResult>> | Promise<Readable<ApolloResult<LoaderResult>>>) | undefined
		fromQuery?: (CreateQueryResult<LoaderResult, LoaderError>) | undefined
		fromInfiniteQuery?: (CreateInfiniteQueryResult<LoaderResult, LoaderError>) | undefined

		then?: ((result: LoaderResult) => LoaderReturnResult) | undefined
		whenLoaded?: ((result: LoaderReturnResult) => void) | undefined
		whenErrored?: ((error: LoaderError) => void) | undefined
		whenCanceled?: (() => Promise<any>) | undefined
		...
		pagination: Pick<InfiniteQueryObserverResult<LoaderResult, LoaderError>, 'hasPreviousPage' | 'isFetchingPreviousPage' | 'fetchPreviousPage' | 'hasNextPage' | 'isFetchingNextPage' | 'fetchNextPage'> | undefined
	...
	// Internal state
	let promise = $state<ReturnType<NonNullable<typeof fromPromise>> | undefined>()
	let store = $state<Awaited<ReturnType<NonNullable<typeof fromStore>>> | undefined>()
	// let houdiniQuery = $state<ReturnType<NonNullable<typeof fromHoudiniQuery>> | undefined>()


	// Outputs
	let _result = $state<LoaderResult>()
	...
	let error = $state<LoaderError>()

	const isHidden = $derived(
		layout === 'headless' || (status === LoadingStatus.Resolved && showIf && !showIf(result))
	)
