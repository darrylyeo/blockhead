...
<InlineContainer class="stack-inline align-{align}" {...$$restProps}>{#key key ?? value}<span class="row-inline align-{align}" transition:transition|local={transitionParams}><slot {key} {value}>{value}</slot></span>{/key}</InlineContainer>
