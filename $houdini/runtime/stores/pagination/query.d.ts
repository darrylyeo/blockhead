...
import { QueryStore, StoreConfig, LoadEventFetchParams, RequestEventFetchParams, QueryStoreFetchParams, ClientFetchParams, QueryResult } from '../query';
import { CursorHandlers } from './cursor';
import { OffsetHandlers } from './offset';
import { PageInfo } from './pageInfo';
declare class CursorPaginatedStore<_Data extends GraphQLObject, _Input> extends QueryStore<_Data, _Input, {
    pageInfo: PageInfo;
}> {
    paginated: boolean;
...
export declare class QueryStoreForwardCursor<_Data extends GraphQLObject, _Input> extends CursorPaginatedStore<_Data, _Input> {
    loadNextPage(args?: Parameters<CursorHandlers<_Data, _Input>['loadNextPage']>[0]): Promise<void>;
}
export declare class QueryStoreBackwardCursor<_Data extends GraphQLObject, _Input> extends CursorPaginatedStore<_Data, _Input> {
    loadPreviousPage(args?: Parameters<Required<CursorHandlers<_Data, _Input>>['loadPreviousPage']>[0]): Promise<void>;
}
export declare class QueryStoreOffset<_Data extends GraphQLObject, _Input> extends QueryStore<_Data, _Input> {
    paginated: boolean;
