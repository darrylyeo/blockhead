...
export async function marshalSelection({ selection, data, }) {
    ...
    return Object.fromEntries(await Promise.all(Object.entries(data).map(async ([fieldName, value]) => {
        var _a;
...
export async function marshalInputs({ artifact, input, rootType = '@root', }) {
    const config = await getCurrentConfig();
    ...
    if (Array.isArray(input)) {
        return await Promise.all(input.map(async (val) => await marshalInputs({ artifact, input: val, rootType })));
    }
    // we're looking at an object, build it up from the current input
    return Object.fromEntries(await Promise.all(Object.entries(input).map(async ([fieldName, value]) => {
        var _a, _b;
        // look up the type for the field
